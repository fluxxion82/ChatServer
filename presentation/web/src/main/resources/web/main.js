...
function connect() {
    ...
    console.log("Begin connect, " + window.location.host);
    ...
    socket.onopen = function() {
        write("Connected");
        socket.send("[{\"date\":" +  new Date().getTime() + ",\"id\":0,\"message\":\"\",\"type\":3,\"username\":\"\"}]")
    };
...
function received(message) {
    ...
    if (chatMessage.type == 3) {
        msg += chatMessage.username + " just joined.";
    } else if (chatMessage.type == 2) {
        msg += chatMessage.username + " has logged out.";
    } else {
        msg += "[" + chatMessage.username + "]:" + chatMessage.message
    }
